...
import { CommentSuggestionRequestDto } from '@/dto/omment-suggestion-request-dto';
...
const showSuggestions = async () => { 
  if (note.value) {
    if (note.value.category && note.value.content) {
      try { 
        // 发送请求
        const commentSuggestionRequestDto:CommentSuggestionRequestDto = {
      ...
      <div class="comments-section">
        ...
        <!-- AI评论建议列表 -->
         <div class="suggestions" v-if="commentSuggestions.length > 0"> 
          <p>推荐评论：</p>
          <div v-for="s in commentSuggestions" :key="s" @click="useSuggestion(s)">{{ s }}</div>
         </div>
